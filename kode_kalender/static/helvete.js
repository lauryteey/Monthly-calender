...
eventForm.addEventListener('submit', async (e) => {
    ...
    // Lagrer dataene fra skjemaet 
    const eventData = {
        dato: document.getElementById('event-date').value.split('T')[0], // Får dato
        klokkeslett: document.getElementById('event-date').value.split('T')[1], // Får time 
        beskrivelse: document.getElementById('event-name').value,
        navn_prosjektet: document.getElementById('event-place').value,
        brukerID: 1, // Example user ID (replace with dynamic value if needed)
    };
